// nnet/nnet-blstm-projected-streams-lc.h
...
#ifndef ASLP_NNET_NNET_BLSTM_PROJECTED_STREAMS_LC_H_
#define ASLP_NNET_NNET_BLSTM_PROJECTED_STREAMS_LC_H_
...
// Latency control blstm componet, the biggest difference is 
// the chunk_size_ compared with the standard blstm component.
...
class BLstmProjectedStreamsLC : public UpdatableComponent {
 public:
  BLstmProjectedStreamsLC(int32 input_dim, int32 output_dim) :
  ...
  ~BLstmProjectedStreamsLC()
  { }

  Component* Copy() const { return new BLstmProjectedStreamsLC(*this); }
  ComponentType GetType() const { return kBLstmProjectedStreamsLC; }
  ...
  void ResetLstmStreams( const std::vector<int32> &stream_reset_flag) {
    // allocate f_prev_nnet_state_ if not done yet,
    if (nstream_ == 0) {
    ...
    // reset flag: 1 - reset stream networt state
  ...
  // set chunk size used for latency controlled BLstm training
  void SetChunkSize( const int32 &chunk_size) {
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    if (do_stream_reset) f_prev_nnet_state_.SetZero();
    ...
    f_prev_nnet_state_.CopyFromMat(f_propagate_buf_.RowRange(chunk_size_*S, S));
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    b_w_gifo_r_corr_.AddMatMat(1.0, B_DGIFO.RowRange(1*S, T*S), kTrans,
                                    B_YR.RowRange(2*S, T*S)   , kNoTrans, mmt);
    // bias of g, i, f, o
