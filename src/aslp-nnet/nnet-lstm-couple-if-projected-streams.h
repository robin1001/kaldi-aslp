// nnet/nnet-lstm-couple-if-projected-streams.h

// Copyright 2016  Shan Yang
// Copyright 2016  Zhang Binbin
// Refined in 2016-08-24
...
#ifndef KALDI_NNET_NNET_LSTM_COUPLE_IF_PROJECTED_STREAMS_H_
#define KALDI_NNET_NNET_LSTM_COUPLE_IF_PROJECTED_STREAMS_H_

#include "aslp-nnet/nnet-component.h"
#include "aslp-nnet/nnet-utils.h"
#include "cudamatrix/cu-math.h"

// Lstm with coupled input gate and forget gate
// i(t) = 1 - f(t)
/*************************************
 * x: input neuron
...
class LstmCifgProjectedStreams : public UpdatableComponent {
 public:
  LstmCifgProjectedStreams(int32 input_dim, int32 output_dim) :
  ...
  ~LstmCifgProjectedStreams()
  { }

  Component* Copy() const { return new LstmCifgProjectedStreams(*this); }
  ComponentType GetType() const { return kLstmCifgProjectedStreams; }
  ...
  void InitData(std::istream &is) {
    ...
    w_gfo_x_.Resize(3*ncell_, input_dim_, kUndefined);
    w_gfo_r_.Resize(3*ncell_, nrecur_, kUndefined);
    w_r_m_.Resize(nrecur_, ncell_, kUndefined);

    InitMatParam(w_gfo_x_, param_scale);
    InitMatParam(w_gfo_r_, param_scale);
    ...
    // init delta buffers
    w_gfo_x_corr_.Resize(3*ncell_, input_dim_, kSetZero);
    w_gfo_r_corr_.Resize(3*ncell_, nrecur_, kSetZero);
  ...
  void ReadData(std::istream &is, bool binary) {
    ...
    w_gfo_x_.Read(is, binary);
    w_gfo_r_.Read(is, binary);
  ...
  void WriteData(std::ostream &os, bool binary) const {
    ...
    w_gfo_x_.Write(os, binary);
    w_gfo_r_.Write(os, binary);
  ...
  int32 NumParams() const {
    return ( w_gfo_x_.NumRows() * w_gfo_x_.NumCols() +
         w_gfo_r_.NumRows() * w_gfo_r_.NumCols() +
         bias_.Dim() +
         peephole_f_c_.Dim() +
         peephole_o_c_.Dim() +
  ...
  void GetParams(Vector<BaseFloat>* wei_copy) const {
    ...
    offset = 0;  len = w_gfo_x_.NumRows() * w_gfo_x_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_gfo_x_);

    offset += len; len = w_gfo_r_.NumRows() * w_gfo_r_.NumCols();
    wei_copy->Range(offset, len).CopyRowsFromMat(w_gfo_r_);
  ...
  void GetGpuParams(std::vector<std::pair<BaseFloat *, int> > *params) {
    params->clear();
    params->push_back(std::make_pair(w_gfo_x_.Data(), w_gfo_x_.NumRows() * w_gfo_x_.Stride()));
    params->push_back(std::make_pair(w_gfo_r_.Data(), w_gfo_r_.NumRows() * w_gfo_r_.Stride()));
  ...
  std::string Info() const {
    return std::string("  ") +
      "\n  w_gfo_x_  "   + MomentStatistics(w_gfo_x_) +
      "\n  w_gfo_r_  "   + MomentStatistics(w_gfo_r_) +
  ...
  std::string InfoGradient() const {
    ...
    const CuSubMatrix<BaseFloat> YF(propagate_buf_.ColRange(1*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> YO(propagate_buf_.ColRange(2*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> YC(propagate_buf_.ColRange(3*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> YH(propagate_buf_.ColRange(4*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> YM(propagate_buf_.ColRange(5*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> YR(propagate_buf_.ColRange(6*ncell_, nrecur_));
    ...
    const CuSubMatrix<BaseFloat> DF(backpropagate_buf_.ColRange(1*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> DO(backpropagate_buf_.ColRange(2*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> DC(backpropagate_buf_.ColRange(3*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> DH(backpropagate_buf_.ColRange(4*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> DM(backpropagate_buf_.ColRange(5*ncell_, ncell_));
    const CuSubMatrix<BaseFloat> DR(backpropagate_buf_.ColRange(6*ncell_, nrecur_));

    return std::string("  ") +
      "\n  Gradients:" +
      "\n  w_gfo_x_corr_  "   + MomentStatistics(w_gfo_x_corr_) +
      "\n  w_gfo_r_corr_  "   + MomentStatistics(w_gfo_r_corr_) +
  ...
  void ResetLstmStreams(const std::vector<int32> &stream_reset_flag) {
    ...
    if (nstream_ == 0) {
      ...
      prev_nnet_state_.Resize(nstream_, 6*ncell_ + 1*nrecur_, kSetZero);
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    propagate_buf_.Resize((T+2)*S, 6 * ncell_ + nrecur_, kSetZero);
    ...
    CuSubMatrix<BaseFloat> YF(propagate_buf_.ColRange(1*ncell_, ncell_));
    CuSubMatrix<BaseFloat> YO(propagate_buf_.ColRange(2*ncell_, ncell_));
    CuSubMatrix<BaseFloat> YC(propagate_buf_.ColRange(3*ncell_, ncell_));
    CuSubMatrix<BaseFloat> YH(propagate_buf_.ColRange(4*ncell_, ncell_));
    CuSubMatrix<BaseFloat> YM(propagate_buf_.ColRange(5*ncell_, ncell_));
    CuSubMatrix<BaseFloat> YR(propagate_buf_.ColRange(6*ncell_, nrecur_));

    CuSubMatrix<BaseFloat> YGFO(propagate_buf_.ColRange(0, 3*ncell_));

    // x -> g, f, o, not recurrent, do it all in once
    YGFO.RowRange(1*S,T*S).AddMatMat(1.0, in, kNoTrans, w_gfo_x_, kTrans, 0.0);
    ...
    // bias -> g, f, o
    YGFO.RowRange(1*S,T*S).AddVecToRows(1.0, bias_);
    ...
    for (int t = 1; t <= T; t++) {
      ...
      CuSubMatrix<BaseFloat> y_gfo(YGFO.RowRange(t*S,S));

      // r(t-1) -> g, f, o
      y_gfo.AddMatMat(1.0, YR.RowRange((t-1)*S,S), kNoTrans, w_gfo_r_, kTrans,  1.0);
      ...
      // g -> c, for traditional lstm, y_c.AddMatMatElements(1.0, y_g, y_i, 0.0);
      //but for coupled input and forget gate, y_i = 1 - y_f
      y_c.AddMatMatElements(-1.0, y_g, y_f, 0.0);
      y_c.AddMat(1.0, y_g);
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    backpropagate_buf_.Resize((T+2)*S, 6 * ncell_ + nrecur_, kSetZero);

    // disassemble backpropagate buffer into neurons
    CuSubMatrix<BaseFloat> DG(backpropagate_buf_.ColRange(0*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DF(backpropagate_buf_.ColRange(1*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DO(backpropagate_buf_.ColRange(2*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DC(backpropagate_buf_.ColRange(3*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DH(backpropagate_buf_.ColRange(4*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DM(backpropagate_buf_.ColRange(5*ncell_, ncell_));
    CuSubMatrix<BaseFloat> DR(backpropagate_buf_.ColRange(6*ncell_, nrecur_));

    CuSubMatrix<BaseFloat> DGFO(backpropagate_buf_.ColRange(0, 3*ncell_));
    ...
    for (int t = T; t >= 1; t--) {
      ...
      //   backprop error from g(t+1), f(t+1), o(t+1) to r(t)
      d_r.AddMatMat(1.0, DGFO.RowRange((t+1)*S,S), kNoTrans, w_gfo_r_, kNoTrans, 1.0);
      ...
      */
      ...
      // 3. diff from f(t+1) (via peephole)
      // 4. diff from o(t)   (via peephole, not recurrent)
      d_c.AddMat(1.0, d_h);
      ...
      d_f.AddMatMatElements(-1.0, d_c, YG.RowRange((t)*S,S), 1.0);
      d_f.DiffSigmoid(y_f, d_f);
      
      // c -> g via input gate, for traditional lstm, d_g = d_c*y_i*g'(a), d_g.AddMatMatElements(1.0, d_c, y_i, 0.0);
      // but for coupled input and forget gate, d_g = d_c*(1-y_f)*g'(a) 
      d_g.AddMatMatElements(-1.0, d_c, y_f, 0.0);
      d_g.AddMat(1.0, d_c);
    ...
    // g,f,o -> x, do it all in once
    in_diff->AddMatMat(1.0, DGFO.RowRange(1*S,T*S), kNoTrans, w_gfo_x_, kNoTrans, 0.0);
    ...
    // weight x -> g, f, o
    w_gfo_x_corr_.AddMatMat(1.0, DGFO.RowRange(1*S,T*S), kTrans,
                                  in                     , kNoTrans, mmt);
    // recurrent weight r -> g, i, f, o
    w_gfo_r_corr_.AddMatMat(1.0, DGFO.RowRange(1*S,T*S), kTrans,
                                  YR.RowRange(0*S,T*S)   , kNoTrans, mmt);
    // bias of g, f, o
    bias_corr_.AddRowSumMat(1.0, DGFO.RowRange(1*S,T*S), mmt);
    ...
    if (clip_gradient_ > 0.0) {
      w_gfo_x_corr_.ApplyFloor(-clip_gradient_);
      w_gfo_x_corr_.ApplyCeiling(clip_gradient_);
      w_gfo_r_corr_.ApplyFloor(-clip_gradient_);
      w_gfo_r_corr_.ApplyCeiling(clip_gradient_);
    ...
    if (DEBUG) {
      ...
      std::cerr << "w_gfo_x_corr_ " << w_gfo_x_corr_;
      std::cerr << "w_gfo_r_corr_ " << w_gfo_r_corr_;
  ...
  void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    w_gfo_x_.AddMat(-lr, w_gfo_x_corr_);
    w_gfo_r_.AddMat(-lr, w_gfo_r_corr_);
...
//
 ...
 private:
  ...
  // feed-forward connections: from x to [g, f, o]
  CuMatrix<BaseFloat> w_gfo_x_;
  CuMatrix<BaseFloat> w_gfo_x_corr_;

  // recurrent projection connections: from r to [g, f, o]
  CuMatrix<BaseFloat> w_gfo_r_;
  CuMatrix<BaseFloat> w_gfo_r_corr_;

  // biases of [g, f, o]
  CuVector<BaseFloat> bias_;
  CuVector<BaseFloat> bias_corr_;

  // peephole from c to f, g
  ...
  // propagate buffer: output of [g, f, o, c, h, m, r]
  CuMatrix<BaseFloat> propagate_buf_;

  // back-propagate buffer: diff-input of [g, f, o, c, h, m, r]
