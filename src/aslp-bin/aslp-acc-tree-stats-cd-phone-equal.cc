...
void AccumulateTreeStatsPhone(const TransitionModel &trans_model,
  ...
  for (int i = -N; i < static_cast<int>(split_alignment.size()); i++) {
    ...
    if (i + P >= 0 && i + P < static_cast<int>(split_alignment.size())) {
      ...
      //binbin split the alignment into 3 equal part
      size_t frame_len = split_alignment[i+P].size();
      size_t step = frame_len / 3;
      std::vector <int> middle;
      middle.push_back((0 + step) / 2);
      //start step: end step+step
      middle.push_back((step + step * 2) / 2); 
      //start 2*step, end: split_alignment[i+P].size()-1
      middle.push_back((2*step + split_alignment[i+P].size()-1) / 2); 
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Accumulate cd-phone statistics using equal alignment for phonetic-context tree building.\n"
        "Usage:  aslp-acc-tree-stats-cd-phone-equal [options] model-in features-rspecifier alignments-rspecifier [tree-accs-out]\n"
        "e.g.: \n"
        " aslp-acc-tree-stats-cd-phone-equal 1.mdl scp:train.scp ark:1.ali 1.tacc\n";
