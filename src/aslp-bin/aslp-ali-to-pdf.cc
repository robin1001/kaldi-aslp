...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      std::vector<int32> new_alignment = alignment;
      std::vector<int32>::iterator it = std::unique(new_alignment.begin(), new_alignment.end());
      new_alignment.erase(it, new_alignment.end());

      writer.Write(key, new_alignment);
      num_done++;
    }
