...
# Mainly copy from utils/mkgraph.sh, but use aslp-make-h3-transducer
# One hmm state continues at least 3 frames
...
if [[ ! -s $dir/Ha.fst || $dir/Ha.fst -ot $model  \
  ...
  if $reverse; then
    aslp-make-h3-transducer --reverse=true --push_weights=true \
      --disambig-syms-out=$dir/disambig_tid.int \
      --transition-scale=$tscale $lang/tmp/ilabels_${N}_${P} $tree $model \
      > $dir/Ha.fst  || exit 1;
  else
    aslp-make-h3-transducer --disambig-syms-out=$dir/disambig_tid.int \
      --transition-scale=$tscale $lang/tmp/ilabels_${N}_${P} $tree $model \
       > $dir/Ha.fst  || exit 1;
  fi
fi
