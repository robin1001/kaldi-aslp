(function(window){
	...
	//var SERVER = "ws://10.13.34.168:10000/ws/decode";
	var SERVER = "ws://localhost:10086/ws/decode";
      ...
      if (config["content_id"]) {
        url += '&content-id=' + config["content_id"]
			...
			ws.onmessage = function(e) {
				...
				} else { // get recognition result
					  //var res = JSON.parse(data);
						var res = eval("(" + data + ")");
					if (res.status == 1) { // partial result
						//config.onEvent(14, res.result);
						if (res.result && !(res.result === "")) {
							config.onPartialResults(res.result);
						} 
					} else if (res.status == 2) { // final result
						if (res.result && !(res.result === "")) {
							config.onResults(res.result);
						}
					} else if (res.status == 4) { // all results were received, we can close websocket now
            ws.close();
					} else if (res.status == 8) { // decoders are not available
            config.onError(ERR_SERVER, 'Server error: decoder error');
					} else if (res.status == 9) { // decoders are not available
            config.onError(ERR_SERVER, 'Server error: decoding service is not available now');
          } else { // unkown
						config.onError(ERR_SERVER, 'Server error: get unknown status from server ' + res.status);
					}
				}
			}
